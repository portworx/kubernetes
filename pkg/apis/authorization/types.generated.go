...
func (x codecSelfer1234) decSliceapi_OwnerReference(v *[]pkg2_api.OwnerReference, d *codec1978.Decoder) {
	...
	if yyl484 == 0 {
		if yyv484 == nil {
			yyv484 = []pkg2_api.OwnerReference{}
			yyc484 = true
		...
		if yyl484 > cap(yyv484) {
			...
			if yyrt484 {
				if yyrl484 <= cap(yyv484) {
					...
					yyv484 = make([]pkg2_api.OwnerReference, yyrl484)
				}
			} else {
				yyv484 = make([]pkg2_api.OwnerReference, yyrl484)
			}
		...
		for ; yyj484 < yyrr484; yyj484++ {
			...
			if r.TryDecodeAsNil() {
				yyv484[yyj484] = pkg2_api.OwnerReference{}
			} else {
		...
		if yyrt484 {
			for ; yyj484 < yyl484; yyj484++ {
				yyv484 = append(yyv484, pkg2_api.OwnerReference{})
				yyh484.ElemContainerState(yyj484)
				if r.TryDecodeAsNil() {
					yyv484[yyj484] = pkg2_api.OwnerReference{}
				} else {
		...
		for ; !r.CheckBreak(); yyj484++ {
			...
			if yyj484 >= len(yyv484) {
				yyv484 = append(yyv484, pkg2_api.OwnerReference{}) // var yyz484 pkg2_api.OwnerReference
				yyc484 = true
			}
			yyh484.ElemContainerState(yyj484)
			if yyj484 < len(yyv484) {
