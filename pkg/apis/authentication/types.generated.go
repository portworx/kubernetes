...
func (x codecSelfer1234) decSliceapi_OwnerReference(v *[]pkg2_api.OwnerReference, d *codec1978.Decoder) {
	...
	if yyl187 == 0 {
		if yyv187 == nil {
			yyv187 = []pkg2_api.OwnerReference{}
			yyc187 = true
		...
		if yyl187 > cap(yyv187) {
			...
			if yyrt187 {
				if yyrl187 <= cap(yyv187) {
					...
					yyv187 = make([]pkg2_api.OwnerReference, yyrl187)
				}
			} else {
				yyv187 = make([]pkg2_api.OwnerReference, yyrl187)
			}
		...
		for ; yyj187 < yyrr187; yyj187++ {
			...
			if r.TryDecodeAsNil() {
				yyv187[yyj187] = pkg2_api.OwnerReference{}
			} else {
		...
		if yyrt187 {
			for ; yyj187 < yyl187; yyj187++ {
				yyv187 = append(yyv187, pkg2_api.OwnerReference{})
				yyh187.ElemContainerState(yyj187)
				if r.TryDecodeAsNil() {
					yyv187[yyj187] = pkg2_api.OwnerReference{}
				} else {
		...
		for ; !r.CheckBreak(); yyj187++ {
			...
			if yyj187 >= len(yyv187) {
				yyv187 = append(yyv187, pkg2_api.OwnerReference{}) // var yyz187 pkg2_api.OwnerReference
				yyc187 = true
			}
			yyh187.ElemContainerState(yyj187)
			if yyj187 < len(yyv187) {
