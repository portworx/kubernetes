...
func (x codecSelfer1234) decSliceapi_OwnerReference(v *[]pkg2_api.OwnerReference, d *codec1978.Decoder) {
	...
	if yyl175 == 0 {
		if yyv175 == nil {
			yyv175 = []pkg2_api.OwnerReference{}
			yyc175 = true
		...
		if yyl175 > cap(yyv175) {
			...
			if yyrt175 {
				if yyrl175 <= cap(yyv175) {
					...
					yyv175 = make([]pkg2_api.OwnerReference, yyrl175)
				}
			} else {
				yyv175 = make([]pkg2_api.OwnerReference, yyrl175)
			}
		...
		for ; yyj175 < yyrr175; yyj175++ {
			...
			if r.TryDecodeAsNil() {
				yyv175[yyj175] = pkg2_api.OwnerReference{}
			} else {
		...
		if yyrt175 {
			for ; yyj175 < yyl175; yyj175++ {
				yyv175 = append(yyv175, pkg2_api.OwnerReference{})
				yyh175.ElemContainerState(yyj175)
				if r.TryDecodeAsNil() {
					yyv175[yyj175] = pkg2_api.OwnerReference{}
				} else {
		...
		for ; !r.CheckBreak(); yyj175++ {
			...
			if yyj175 >= len(yyv175) {
				yyv175 = append(yyv175, pkg2_api.OwnerReference{}) // var yyz175 pkg2_api.OwnerReference
				yyc175 = true
			}
			yyh175.ElemContainerState(yyj175)
			if yyj175 < len(yyv175) {
